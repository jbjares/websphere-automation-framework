...
//	private static final String PLUGIN = "PLUGIN";
	...
	public <T> List<T> parse(T arg, String fileName) throws FileNotFoundException, IOException, IllegalArgumentException, IllegalAccessException, SecurityException, InstantiationException, InvocationTargetException, NoSuchMethodException {
		...
		int maxNumberOfItens = getMaxNumberOfItensForList(arg.getClass().getDeclaredFields());
		for(int i=0;i<maxNumberOfItens;i++){
			...
			for(Field field : obj.getClass().getDeclaredFields()){		
				...
				if(isStringType){
					List<String> stringsAttr = (List<String>)getValorAtributoLMLista(annField.name());
					...
					if(stringsAttr.size()>1){
						for(String stringAttr: stringsAttr){
							field.set(obj,stringAttr);
							stringsAttr.remove(stringAttr);
							break;
						}						
					}
				...
				if(isBooleanType){
					List<String> booleansAttr = (List<String>)getValorAtributoLMLista(annField.name());
				...
				if(isList){
					...
					if(isListOfTypeString){
						if(getValorAtributoLMLista(annField.name())!=null){
							List<String> listAttr = (List<String>) getValorAtributoLMLista(annField.name());
	...
	private int getMaxNumberOfItensForList(Field[] declaredFields) {
		for(Field field : declaredFields){
			ODMColumn annField = field.getAnnotation(ODMColumn.class);
			if(annField!=null && annField.required()){
				return getValorAtributoLMLista(annField.name()).size();
			}
		}
	...
	private void validarAnotacaoClasse(ODMEntity annType) {
		if(annType==null){throw new RuntimeException("Nenhuma anotação definida para a classe informada");}
//		if(!DOC.equals(annType.docType())){
//			throw new RuntimeException("O componente autalmente suporta somente o tipo doc");
//		}
	}

	public abstract String getValorAtributoLM(String atributo);
	
	public abstract List<?> getValorAtributoLMLista(String atributo);
